...
function validateBookingDate(showMessage = true) {
  if (!date || !date.value) return true;
  if (!isBlockedBookingDay(date.value)) return true;

  date.value = "";
  if (showMessage && message) {
    message.textContent = "Online scheduling is unavailable Wednesday through Friday. Please choose Monday, Tuesday, Saturday, or Sunday.";
...
if (date) date.addEventListener("change", () => validateBookingDate());
